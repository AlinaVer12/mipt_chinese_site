...
def words_trainer(request):
    ...
    return render(request, "word_trainer.html",
                  context={"train_words": train_words})


def check_word(request):
    """
    Проверка ответа при повторении в тренажере
    """
    if request.method == "POST":
        cache.clear()
        ...
        context = {"true_index)": true_index, "train_character": train_character,
                   "train_translation": train_translation}
        if user_index not in ["1",'2','3']:
            context["success"] = False
            context["comment"] = ("Введено неверное значение. "
                                  "Ваш ответ должен быть числом (1, 2 или 3).")
